...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const mkt of marketTypes) {
      try {
        ...
        for (const player of odds) {
          ...
          const modelProb: number = player.datagolf?.baseline_history_fit || player.datagolf?.baseline || 0;
          ...
          for (const book of books) {
            ...
            // Edge = where model thinks player is MORE likely than the book
            // This means the book is underpricing the player = value bet
    ...
    // Dedupe: keep best edge per player+market
    ...
    edges.sort((a, b) => tierOrder[a.tier] !== tierOrder[b.tier] ? tierOrder[a.tier] - tierOrder[b.tier] : b.edge - a.edge);
...
ENDOFFILEcd ~/Downloads/golf-edge-finder && cat > src/app/api/scan/route.ts << 'ENDOFFILE'
import { NextRequest, NextResponse } from 'next/server';
import { sendEdgeAlerts } from '@/lib/notify';
import { MarketType, EdgeOpportunity, EdgeTier, TournamentEdgeScan, DEFAULT_THRESHOLDS } from '@/types';
